...
export async function GET(req: Request) {
  try {
    const url = backendUrl("/enterprise/runs", req);
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "runs_proxy_failed", status: res.status, body: text.slice(0, 500) },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({ error: "runs_proxy_exception", message: e?.message || "unknown" }, { status: 500 });
  }
}
