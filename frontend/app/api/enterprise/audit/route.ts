...
export async function GET(req: Request) {
  try {
    const url = backendUrl("/enterprise/audit", req);
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "audit_proxy_failed", status: res.status, body: text.slice(0, 500) },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({ error: "audit_proxy_exception", message: e?.message || "unknown" }, { status: 500 });
  }
}
