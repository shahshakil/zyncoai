  ...
  return (
    <section className="py-20">
      ...
      <div className="mx-auto max-w-3xl text-center">
        <h2 className="text-3xl font-bold text-[#f8fafc] sm:text-4xl">Hear Charlotte answer a real call</h2>
        <p className="mt-3 text-[#94a3b8]">Press play — this is the real Charlotte voice, the same one that answers every ZyncoAI call.</p>
      </div>
      ...
      <div className="mx-auto mt-10 max-w-3xl rounded-3xl border border-white/10 bg-[#0b0f19] p-6 sm:p-8">
        <div className="flex items-center justify-between">
          <p className="text-xs uppercase tracking-wide text-[#94a3b8]">Example conversation</p>
          <button
            onClick={togglePlay}
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#4f87f0] to-[#7c3aed] px-4 py-2 text-sm font-semibold text-white hover:opacity-90"
          >
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex ${isCaller ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm transition-shadow ${
                    isCaller ? "bg-slate-100 text-[#0f172a]" : "bg-[#1e293b] text-[#e2e8f0]"
                  } ${isSpeaking ? "ring-2 ring-[#4f87f0] shadow-[0_0_20px_rgba(79,135,240,0.45)]" : ""}`}
                >
                  <p className={`mb-0.5 text-[10px] uppercase tracking-wide ${isCaller ? "text-[#64748b]" : "text-[#94a3b8]"}`}>
        ...
        <div className="mt-6 flex justify-center">
          <Link href="/demo" className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#8ab4ff] hover:text-[#a9c6ff]">
        ...
        <div className="mt-8 grid grid-cols-3 gap-4 border-t border-white/10 pt-6 text-center">
          <div>
            <div className="text-xl font-bold text-[#f8fafc]"><CountUp target={847} /></div>
            <p className="mt-1 text-xs text-[#94a3b8]">Calls answered today</p>
          </div>
          <div>
            <div className="text-xl font-bold text-[#f8fafc]"><CountUp target={312} /></div>
            <p className="mt-1 text-xs text-[#94a3b8]">Bookings made</p>
          </div>
          <div>
            <div className="text-xl font-bold text-[#f8fafc]">0.8s</div>
