    ...
    render: () => (
      <div className="flex flex-col items-center gap-3">
        <div className="max-w-[280px] rounded-2xl rounded-bl-sm bg-slate-100 px-4 py-3 text-left text-sm text-[#0f172a]">
          <p className="mb-1 flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-[#64748b]">
  ...
  },
  {
    ...
    render: () => (
      <div className="flex w-full max-w-[280px] flex-col gap-2 rounded-xl border border-[#e2e8f0] bg-white p-3 text-left shadow-sm">
        <div className="flex items-center justify-between text-[10px] uppercase tracking-wide text-[#94a3b8]">
          <span className="flex items-center gap-1"><LayoutDashboard className="h-3 w-3" /> Bookings</span>
          <span className="flex items-center gap-1 text-emerald-600"><CheckCircle2 className="h-3 w-3" /> Synced</span>
        </div>
  ...
  return (
    <section className="py-20">
      <div className="mx-auto max-w-2xl text-center">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#94a3b8]">How it works</p>
      ...
      <div className="mx-auto mt-12 max-w-lg overflow-hidden rounded-2xl border border-[#e2e8f0] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex items-center gap-2 border-b border-[#e2e8f0] bg-slate-50 px-4 py-2.5">
          ...
          <span className="ml-2 text-xs font-medium text-[#64748b]">Live Call Flow</span>
        </div>

        {/* content */}
        <div className="relative flex min-h-[220px] items-center justify-center overflow-hidden px-6 py-10">
          <motion.div
            className="pointer-events-none absolute text-[#94a3b8]"
        ...
        {/* progress-dot rail */}
        <div className="flex items-center justify-center gap-2 border-t border-[#e2e8f0] bg-slate-50 py-3">
          {STEPS.map((s, i) => (
            <button
              key={s.label}
              onClick={() => setStep(i)}
      ...
      <p className="mx-auto mt-4 max-w-lg text-center text-xs text-[#94a3b8]">{current.label}</p>
    </section>
  );
}
