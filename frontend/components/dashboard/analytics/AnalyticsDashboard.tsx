...
// Dark glass styling matches DynamicAnalyticsPanel/LiveAgentHud — the
// premium-dark treatment established for this app's "live system" panels,
// applied to the whole page here per spec.
...
function GlassCard({ children, className = "", style }: { children: React.ReactNode; className?: string; style?: React.CSSProperties }) {
  return (
    <div
      className={`rounded-2xl border border-slate-800/60 bg-slate-950/80 p-5 shadow-xl backdrop-blur-md transition-all duration-300 hover:-translate-y-[2px] hover:shadow-[0_0_30px_-8px_var(--panel-glow)] ${className}`}
      style={style}
    >
      {children}
    </div>
...
function KpiCard({ label, value, delta, icon: Icon, accent }: { label: string; value: React.ReactNode; delta?: number | null; icon: any; accent: string }) {
  return (
    <GlassCard className="p-4" style={{ ["--panel-glow" as any]: accent }}>
  ...
  if (isLoading || !data) {
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-6">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-28 w-full bg-slate-800/60" />)}</div>
        <Skeleton className="h-72 w-full bg-slate-800/60" />
      </div>
    );
  }

  return (
    <div className="space-y-6" style={{ ["--panel-glow" as any]: theme.accent }}>
      {/* Header + export bar */}
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => triggerPrint("pdf")} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-800">
            <Download className="h-3.5 w-3.5" /> Download PDF
          </button>
          <button onClick={exportExcel} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-800">
          ...
          {role === "OWNER" && (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition disabled:opacity-50 ${
                schedule?.schedule.active ? "border-emerald-600/40 bg-emerald-500/10 text-emerald-300" : "border-slate-700 bg-slate-900 text-slate-200 hover:bg-slate-800"
              }`}
            >
            ...
            return (
              <div key={s.key}>
                <div className="mb-1 flex items-center justify-between text-xs">
                  <span className={isDropOff ? "font-semibold text-rose-300" : "text-slate-300"}>{s.label}{isDropOff && " — biggest drop-off"}</span>
      ...
      {/* D. Hourly heatmap */}
      <GlassCard className="no-print overflow-x-auto">
          ...
          {data.patients.topSuburbs.length === 0 ? (
            ...
            <div className="space-y-2">
              {data.patients.topSuburbs.map((s) => {
                ...
                return (
                  <div key={s.suburb}>
                    <div className="mb-1 flex justify-between text-xs text-slate-300"><span>{s.suburb}</span><span className="text-slate-400">{s.count}</span></div>
                    <div className="h-2 rounded-full bg-slate-800/60"><div className="h-full rounded-full" style={{ width: `${(s.count / max) * 100}%`, background: theme.accent }} /></div>
                  </div>
                );
              })}
            </div>
        ...
        <GlassCard className="no-print">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <LineChart data={data.patients.noShowRateTrend}>
              <XAxis dataKey="weekLabel" stroke="#64748b" fontSize={10} tickLine={false} axisLine={false} interval={2} />
        ...
        <GlassCard className="no-print">
          ...
          {!data.patients.recallSuccess ? (
            ...
            <div className="flex items-center gap-6 py-4">
              <p className="text-4xl font-bold tabular-nums text-white">{data.patients.recallSuccess.ratePct}%</p>
      ...
      <GlassCard className="no-print">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data.revenue.monthlyTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <YAxis stroke="#64748b" fontSize={11} tickLine={false} axisLine={false} width={40} tickFormatter={(v) => `$${Math.round(v / 100)}`} />
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-slate-900/60 p-3">
            ...
            <p className="mt-1 text-xl font-semibold text-white">{money(data.revenue.revenuePerBookingCents)}</p>
          </div>
          <div className="rounded-xl bg-slate-900/60 p-3">
            <p className="text-[11px] uppercase tracking-wide text-slate-400">Projected Next Month</p>
            <p className="mt-1 text-xl font-semibold text-white">{money(data.revenue.projectedNextMonthRevenueCents)}</p>
          ...
          <div className="rounded-xl bg-slate-900/60 p-3">
            ...
            {data.revenue.roiMultiple != null ? (
              <p className="mt-1 text-xl font-semibold text-emerald-400">{data.revenue.roiMultiple}×</p>
            ) : (
      ...
      {row.map((count, h) => {
        ...
        return (
          <div
            ...
            style={{ background: count === 0 ? "#1e293b" : `rgba(${rgb}, ${0.15 + intensity * 0.8})` }}
          />
        );
      })}
    </>
...
function ScorecardTile({ label, unit, data, color }: { label: string; unit: string; data: number[]; color: string }) {
  ...
  return (
    <div className="rounded-xl bg-slate-900/60 p-3">
      ...
      <p className="mt-1 text-xl font-bold tabular-nums text-white">{latest != null ? `${Math.round(latest).toLocaleString()}${unit}` : "—"}</p>
