...
const INTENT_COLOR: Record<LiveCall["intent"], string> = {
  booking_request: "bg-blue-500/15 text-blue-300",
  checking_calendar: "bg-amber-500/15 text-amber-300",
  confirming_details: "bg-emerald-500/15 text-emerald-300",
  assisting: "bg-indigo-500/15 text-indigo-300",
};
    ...
    <div className="flex h-4 items-end gap-0.5">
      {[0, 1, 2, 3, 4].map((i) => (
        <span
          ...
          style={{ animation: `zynco-eq 0.8s ease-in-out ${i * 0.1}s infinite`, height: "40%" }}
        />
      ))}
    </div>
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {activeCalls.length > 0 && (
          <div className="space-y-2 rounded-2xl border border-indigo-500/30 bg-slate-950/80 p-3 shadow-[0_0_20px_-4px_rgba(99,102,241,0.5)] backdrop-blur-md">
            <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-indigo-300">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-indigo-400" /> AI Agent — Live
            </div>
            {activeCalls.map((c) => (
              <div key={c.id} className="animate-pulse-border rounded-xl border border-indigo-500/40 bg-white/[0.03] p-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <Equalizer />
                    <span className="truncate text-xs font-medium text-white">{c.contactName || c.fromNumber}</span>
                  </div>
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${INTENT_COLOR[c.intent]}`}>{INTENT_LABEL[c.intent]}</span>
                </div>
                {c.lastLine && (
                  <p key={c.lastLine.text} className="mt-1.5 truncate text-[11px] text-slate-400 animate-in fade-in duration-500">
